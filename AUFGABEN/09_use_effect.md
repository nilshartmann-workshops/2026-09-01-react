...
1. Leg `src/spielwiese/EffektSpielwiese.tsx` an und häng sie in die Anwendung.
   - Die Komponente zeigt den `Counter` (Default-Export aus
     `src/spielwiese/Counter.tsx`) und darüber einen Button, mit dem du ihn aus-
     und wieder einblenden kannst. Den Button brauchst du gleich: Ohne ihn
     bekommst du die Aufräum-Funktion nie zu sehen.
   - Trag in `src/components/App.tsx` einen weiteren `Tab` mit der Beschriftung
     "Effekte (Spielwiese)" und das passende `Panel` dazu ein, so wie beim Tab
     davor und mit einer eigenen `tabId`.
   - Bis der Effekt drin ist, zählt der `Counter` nur vor sich hin. Das ist
     richtig so.
...
7. 👀 **Das schauen wir uns gemeinsam an, du musst es nicht selbst bauen:** ein zweiter Effekt, der mit `setInterval` jede Sekunde etwas auf die Konsole schreibt, zuerst ohne Aufräum-Funktion.
   - Blendet man den `Counter` ein paar Mal aus und wieder ein, läuft
     anschließend nicht *ein* Timer, sondern für jedes Einblenden einer. Die
     Konsole füllt sich entsprechend schnell.
   - Mit `clearInterval` im Cleanup ist der Spuk vorbei. Bei Timern, Abos und offenen Verbindungen ist das Aufräumen deshalb Pflicht: Anders als beim Tab-Titel bleibt hier etwas **laufen**, nicht nur stehen.
